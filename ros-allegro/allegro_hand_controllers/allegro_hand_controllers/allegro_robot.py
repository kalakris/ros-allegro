...
class AllegroRobot(Node):
    def __init__(self, hand_topic_prefix="allegroHand_0", num_joints=16):
        ...
        self.pub_envelop_torque = self.create_publisher(
            Float32, topic_envelop_torque, 1
        )
        self.create_subscription(JointState, topic_joint_state, self._joint_state_callback, 10)
    ...
    def command_hand_configuration(self, hand_config):
        ...
        if hand_config in self._named_grasps_mappings:
            ...
            self.get_logger().warn("Unable to command unknown grasp {}".format(hand_config))
            return False
