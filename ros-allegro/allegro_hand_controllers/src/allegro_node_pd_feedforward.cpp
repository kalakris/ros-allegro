...
#include "allegro_node_pd_feedforward.h"
#include <stdio.h>
...
AllegroNodePDFeedForward::AllegroNodePDFeedForward(const std::string nodeName, bool sim)
  ...
  lib_cmd_sub = create_subscription<std_msgs::msg::String>(
          LIB_CMD_TOPIC, 1, std::bind(&AllegroNodePDFeedForward::libCmdCallback, this, std::placeholders::_1));

  joint_cmd_sub = create_subscription<sensor_msgs::msg::JointState>(
          DESIRED_STATE_TOPIC, 1, std::bind(&AllegroNodePDFeedForward::setJointCallback, this, std::placeholders::_1));
}

AllegroNodePDFeedForward::~AllegroNodePDFeedForward() {
  delete pBHand;
  RCLCPP_INFO(get_logger(), "PD controller node is shutting down");
}

// Called when an external (string) message is received
void AllegroNodePDFeedForward::libCmdCallback(const std_msgs::msg::String::SharedPtr msg) {
...
void AllegroNodePDFeedForward::computeDesiredTorque() {
...
void AllegroNodePDFeedForward::initController(const std::string &whichHand) {
  // Initialize BHand controller
  if (whichHand.compare("left") == 0) {
    pBHand = new BHand(eHandType_Left);
    RCLCPP_WARN(get_logger(), "CTRL: Left Allegro Hand controller initialized.");
  }
  else {
...
void AllegroNodePDFeedForward::doIt(bool polling) {
  // Main spin loop, uses the publisher/subscribers.
  auto this_node = std::shared_ptr<AllegroNodePDFeedForward>(this);
...
int main(int argc, char **argv) {
  ...
  AllegroNodePDFeedForward allegroNode("allegro_node_pd_feedforward", is_sim);
